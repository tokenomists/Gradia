        ...
        const detailsPromises = userData.classes.map(async (classItem) => {
          try {
            const response = await axios.get(
              `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/classes/${classItem._id}`,
            ...
            console.error(`Error fetching details for class ${classItem._id}:`, err);
            return null;
          }
        });
            ...
            <div className="p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {user.classes.map((classItem, index) => (
                  <div 
                    key={index} 
                    className="bg-[#fcf9ea] rounded-lg p-4 hover:shadow-md transition-shadow"
                  >
